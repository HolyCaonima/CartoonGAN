...
def main():
    ...
    for epoch in range(start_epoch, FLAGS.max_epoch):
        ...
        for update in pbar(range(FLAGS.updates_per_epoch)):
            gan.update_params(FLAGS.global_step, FLAGS.d_iter, FLAGS.g_iter)
            FLAGS.global_step = FLAGS.global_step + 1
        
        dl, gl = gan.get_loss()
        print "loss: " + str([dl,gl])
        gan.generate_and_save_images((int(math.sqrt(FLAGS.batch_size))+1)**2, FLAGS.working_directory)
